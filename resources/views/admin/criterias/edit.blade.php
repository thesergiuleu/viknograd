...
@section('title', trans('actions.pages-title.criteria.edit'))

@section('content')
    <main class="main-container">
        <form method="post" action="{{route('criteria.update', $item->id)}}">
                    ...
                    <div class="form-group">
                        ...
                        <select name="supper_criteria_id" class="form-control">
                            @foreach ($supper_criterias as $key => $supper_criteria)
                                <option value="{{$key}}" {{($item->supper_criteria_id === $key ? 'selected' : '')}}>
                                    {{$supper_criteria}}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-2">
                    ...
                    <div class="checkbox">
                        <label>
                            ...
                            <input type="checkbox" name="is_active" value="1" {{$item->is_active == 1 ? 'checked' : ''}}> {{trans('forms.criteria.is_active')}}
                        </label>
                    </div>
                </div>
            </div>
